		...
		switch ( $current_tab ) {
			case 'meta-details':
				self::loadTemplate(
					'admin/meta-details',
					array(
						'detailed_data' => get_option( 'metafiller_meta_data', array() )['detailed_data'] ?? array(),
					)
				);
				break;
			...
			case 'meta-merge':
				self::loadTemplate(
					'admin/meta-merge',
					array(
						'summary_data'   => get_option( 'metafiller_meta_data', array() )['summary_data'] ?? array(),
			...
			case 'settings':
				self::loadTemplate( 'admin/settings', array() );
				break;
			...
			default:
				...
				self::loadTemplate(
					...
					array(
						...
						'active_plugins' => $conflicts['active_plugins'] ?? array(),
					)
				);
				break;
		}
