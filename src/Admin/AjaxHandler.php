...
class AjaxHandler {
	public function __construct() {
		add_action( 'wp_ajax_metafiller_save_meta_field', array( $this, 'saveMetaField' ) );
		add_action( 'wp_ajax_metafiller_generate_metas', array( $this, 'generateMetas' ) );
	}
	...
	public function saveMetaField() {
		...
		if ( empty( $post_id ) || empty( $field ) || empty( $value ) ) {
			wp_send_json_error( array( 'message' => 'Missing or invalid input.' ) );
		}
		...
		if ( $success ) {
			...
			wp_send_json_error( array( 'message' => 'Failed to update meta field.' ) );
		}
	}
	...
	public function generateMetas() {
		...
		$supported_plugins = array( 'Yoast SEO', 'Rank Math' );
