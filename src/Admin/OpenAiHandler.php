...
class OpenAiHandler {
	...
	public function generateMetaTitle( $title, $content ) {
		...
		$messages = array(
			array(
				'role'    => 'system',
				'content' => "You are an assistant specialized in SEO meta title generation. Generate outputs in $language.",
			),
			array(
				'role'    => 'user',
				'content' => "Generate a concise, compelling meta title for the following:\n\nTitle: $title\nContent: $content\n\nIt must be no longer than 60 characters.",
			),
		);
	...
	public function generateMetaDescription( $title, $content ) {
		...
		$messages = array(
			array(
				'role'    => 'system',
				'content' => "You are an assistant specialized in SEO meta description generation. Generate outputs in $language.",
			),
			array(
				'role'    => 'user',
				'content' => "Generate a concise, engaging meta description for the following:\n\nTitle: $title\nContent: $content\n\nIt must be no longer than 160 characters.",
			),
		);
